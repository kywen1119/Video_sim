...
def train(args):
    ...
    def train_step(inputs):
        ...
        with tf.GradientTape() as tape:
            predictions, loss = model(inputs, training=True)
    ...
    def val_step(inputs):
        vids = inputs['vid']
        labels = inputs['mask_labels']
        predictions, loss = model(inputs, training=False)
        # loss = loss_object(labels, predictions) * labels.shape[-1]  # convert mean back to sum
        ...
        for train_batch in train_dataset:
            ...
            if step % args.eval_freq == 0:
                ...
                for val_batch in val_dataset:
                    vids, losses = val_step(val_batch)
                    for vid, loss in zip(vids.numpy(), losses.numpy()):
