...
from cqrmetrics_simcse import Recorder
...
def train(args):
    ...
    def train_step(inputs):
        ...
        with tf.GradientTape() as tape:
            predictions, embeddings, vision_embedding, bert_embedding = model(inputs, training=True)
            _, embeddings_2, _, _ = model(inputs, training=True)
            ...
            loss_simcse = contrastive_loss(embeddings, embeddings_2) * labels.shape[-1]
            loss = loss_0 + loss_1 + loss_simcse 
        gradients = tape.gradient(loss, model.get_variables())
        model.optimize(gradients)
        train_recorder.record(loss, loss_0, loss_1, loss_simcse, labels, predictions)
    ...
    def val_step(inputs):
        ...
        loss_simcse = contrastive_loss(embeddings, embeddings_2) * labels.shape[-1]
        loss = loss_0 + loss_1 + loss_simcse 
        val_recorder.record(loss,loss_0, loss_1, loss_simcse, labels, predictions)
