...
from cqrmetrics_category import Recorder
from cqrmodel_category import MultiModal
...
def train(args):
    ...
    loss_object_1 = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True, reduction=tf.keras.losses.Reduction.NONE)
    ...
    def train_step(inputs):
        ...
        labels_cate_1 = inputs['category_id_1']
        labels_cate_2 = inputs['category_id_2']
        with tf.GradientTape() as tape:
            predictions, _, vision_embedding, bert_embedding, pre_cate_1, pre_cate_2 = model(inputs, training=True)
    ...
    def val_step(inputs):
        ...
        labels_cate_1 = inputs['category_id_1']
        labels_cate_2 = inputs['category_id_2']
        predictions, embeddings, vision_embedding, bert_embedding, pre_cate_1, pre_cate_2 = model(inputs, training=False)
        loss_0 = loss_object(labels, predictions) * labels.shape[-1]  # convert mean back to sum
        loss_1 = contrastive_loss(vision_embedding, bert_embedding) * 10.0
        loss_cate_1 = loss_object_1(labels_cate_1, pre_cate_1) * 10.0#* labels.shape[-1]
        loss_cate_2 = loss_object_1(labels_cate_2, pre_cate_2) * 10.0#* labels.shape[-1]
