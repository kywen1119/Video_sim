...
class MultiModal_mix_addtf(Model):
    def __init__(self, config, *args, **kwargs):
        ...
        self.video_tf_1 = Video_transformer(num_hidden_layers=1, output_size=config.frame_embedding_size, 
                                        seq_len=config.max_frames, dropout=config.dropout)
        self.fusion_vis_1 = ConcatDenseSE(config.vlad_hidden_size, config.se_ratio)
        ...
        self.video_tf_2 = Video_transformer(num_hidden_layers=1, output_size=config.frame_embedding_size, 
                                        seq_len=config.max_frames, dropout=config.dropout)
        self.fusion_vis_2 = ConcatDenseSE(config.vlad_hidden_size, config.se_ratio)
        ...
        self.video_tf_3 = Video_transformer(num_hidden_layers=1, output_size=config.frame_embedding_size, 
                                        seq_len=config.max_frames, dropout=config.dropout)
        self.fusion_vis_3 = ConcatDenseSE(config.vlad_hidden_size, config.se_ratio)
    ...
    def call(self, inputs, **kwargs):
        ...
        # 1
        video_tf_embedding_1, _ = self.video_tf_1([inputs['frames'], frame_num]) # b,32,1536
        ...
        visual_emb_1 = self.fusion_vis_1([vision_embedding_1, video_tf_embedding_1])
        final_embedding_1 = self.fusion_1([visual_emb_1, bert_embedding])
        logits_1 = self.classifier_1(final_embedding_1)
        predictions_1 = tf.nn.sigmoid(logits_1)
        # 2
        video_tf_embedding_2, _ = self.video_tf_2([inputs['frames'], frame_num]) # b,32,1536
        ...
        visual_emb_2 = self.fusion_vis_2([vision_embedding_2, video_tf_embedding_2])
        final_embedding_2 = self.fusion_2([visual_emb_2, bert_embedding])
        logits_2 = self.classifier_2(final_embedding_2)
        predictions_2 = tf.nn.sigmoid(logits_2)
        # 3
        video_tf_embedding_3, _ = self.video_tf_3([inputs['frames'], frame_num]) # b,32,1536
        video_tf_embedding_3 = tf.reduce_max(video_tf_embedding_3, 1)
        ...
        visual_emb_3 = self.fusion_vis_3([vision_embedding_3, video_tf_embedding_3])
        final_embedding_3 = self.fusion_3([visual_emb_3, bert_embedding])
    ...
    def get_variables(self):
        if not self.all_variables:  # is None, not initialized
            ...
            self.normal_variables = self.nextvlad_1.trainable_variables + self.fusion_1.trainable_variables + \
                                    ...
                                    self.video_tf_1.trainable_variables + self.fusion_vis_1.trainable_variables + \
                                    self.video_tf_2.trainable_variables + self.fusion_vis_2.trainable_variables + \
                                    self.video_tf_3.trainable_variables + self.fusion_vis_3.trainable_variables
...
class MultiModal_mix5(Model):
    ...
    def call(self, inputs, **kwargs):
        ...
        predictions_3 = tf.nn.sigmoid(logits_3)
        # 4
        vision_embedding_4 = self.nextvlad_4([inputs['frames'], frame_num])
        vision_embedding_4 = vision_embedding_4 * tf.cast(tf.expand_dims(frame_num, -1) > 0, tf.float32)
        final_embedding_4 = self.fusion_4([vision_embedding_4, bert_embedding])
        logits_4 = self.classifier_4(final_embedding_4)
        predictions_4 = tf.nn.sigmoid(logits_4)
        # 5
        vision_embedding_5 = self.nextvlad_5([inputs['frames'], frame_num])
        vision_embedding_5 = vision_embedding_5 * tf.cast(tf.expand_dims(frame_num, -1) > 0, tf.float32)
        final_embedding_5 = self.fusion_5([vision_embedding_5, bert_embedding])
        logits_5 = self.classifier_3(final_embedding_5)
        predictions_5 = tf.nn.sigmoid(logits_5)
        # mix
        aux_preds = [predictions_1, predictions_2, predictions_3, predictions_4, predictions_5]
        logits = [logits_1, logits_2, logits_3, logits_4, logits_5]
        logits = tf.stack(logits, axis=1)
        embeddings = [final_embedding_1, final_embedding_2, final_embedding_3, final_embedding_4, final_embedding_5]
        embeddings = tf.stack(embeddings, axis=1)
        # vision
        # vision_embedding = [vision_embedding_1, vision_embedding_2, vision_embedding_3]
        # vision_embedding = tf.stack(vision_embedding, axis=1)
        vision_embedding = [vision_embedding_1, vision_embedding_2, vision_embedding_3, vision_embedding_4, vision_embedding_5]
        # vision_embedding = tf.stack(vision_embedding, axis=1)
        # mix_vision_embedding = tf.reduce_sum(tf.multiply(tf.expand_dims(mix_weights, -1), vision_embedding), axis=1)
        mix_logit = tf.reduce_sum(tf.multiply(tf.expand_dims(mix_weights, -1), logits), axis=1)
        mix_embedding = tf.reduce_sum(tf.multiply(tf.expand_dims(mix_weights, -1), embeddings), axis=1)
    ...
    def get_variables(self):
        if not self.all_variables:  # is None, not initialized
            ...
            self.normal_variables = self.nextvlad_1.trainable_variables + self.fusion_1.trainable_variables + \
                                    ...
                                    self.classifier_2.trainable_variables + self.nextvlad_3.trainable_variables + \
                                    self.fusion_3.trainable_variables + self.classifier_3.trainable_variables + \
                                    self.nextvlad_4.trainable_variables + \
                                    self.fusion_4.trainable_variables + self.classifier_4.trainable_variables + \
                                    self.nextvlad_5.trainable_variables + \
                                    self.fusion_5.trainable_variables + self.classifier_5.trainable_variables
            self.all_variables = self.bert_variables + self.normal_variables
        return self.all_variables
